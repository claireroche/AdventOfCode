...
def updateAdjDistances(distMap, states, tmp_weight, pos1, pos2, previousPos):
    if pos2 in distMap:
        ...
        elif distMap[pos2] == distMap[pos1] + weight:
            previousPos[pos2].append(pos1)
...
def buildPaths(previousPos, start, end):
    path = []
    heap = [end]
    while len(heap) > 0:
        cell = heap.pop()
...
def solve(input_file):
    ...
    # build the path
    path = buildPaths(previousPos, start, end)
...
assert(solve("day16_data_test.txt")==45)
assert(solve("day16_data_test2.txt")==64)
#assert(solve("day16_data_test3.txt")==21148)
assert(solve("day16_data_test4.txt")==264)
#assert(solve("day16_data_test5.txt")==4013)

# solve puzzle
print("Number of tiles in at least one of the best paths through the maze:", solve("day16_data.txt"))